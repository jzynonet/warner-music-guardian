...
class Database:
    ...
    def add_video(self, video: Video) -> Optional[int]:
        ...
        try:
            ...
            ''', (video.video_id, video.title, video.channel_name, video.channel_id,
                  ...
                  int(video.auto_flagged), video.created_at))
    ...
    def add_song(self, song_name: str, artist_name: str, artist_id: int = None, 
        ...
        try:
            cursor.execute('''
                INSERT INTO songs (song_name, artist_name, active, artist_id, auto_flag, priority, duration_ms, created_at)
                VALUES (?, ?, 1, ?, ?, ?, ?, ?)
            ''', (song_name, artist_name, artist_id, int(auto_flag), priority, duration_ms, datetime.now().isoformat()))
